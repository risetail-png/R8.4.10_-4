...
# ── 径間番号検出用パターン ──────────────────────────────────────────────────
# ヘッダーに単独数字（1 または 2 など）で記載 or タイトルに「N径間」と記載
RE_SPAN_HEADER   = re.compile(r'(?:^|\s)([1-9]\d?)\s*(?:\n|$)', re.MULTILINE)
RE_SPAN_TITLE    = re.compile(r'([1-9]\d?)\s*径間')
...
def get_span_number_from_text(text):
    """
    ページのテキストから径間番号（整数）を返す。
    見つからない場合は None。

    検出ロジック:
      1. タイトル行の「N径間」
      2. ヘッダー中央付近の単独数字（1 or 2 など）
         ※ ページ番号欄に現れる小さな数字と区別するため、
           「56」「0092」等の路線番号は除外する
    """
    text = _normalize_text(text)

    # 1. タイトル行「N径間」
    m = RE_SPAN_TITLE.search(text)
    if m:
        return int(m.group(1))

    # 2. ヘッダーの単独数字
    #    このPDFでは径間番号はページ上部に「1」「2」と単独で現れる
    #    路線番号(56)・管理番号(0092)は除外
    EXCLUDE = {'56', '0092', '33', '132', '2021', '2016', '2022', '2015'}
    for m in RE_SPAN_HEADER.finditer(text):
        val = m.group(1).strip()
        if val not in EXCLUDE and int(val) < 20:   # 径間番号は常識的に20未満
            return int(val)

    return None


def get_span_number_ocr(pdf_path, page_idx, dpi):
    """OCRでページから径間番号を取得する（テキスト抽出で取れなかった場合用）。"""
    ...
    return get_span_number_from_text(t)
...
def _parse_diag_photo_nums(text):
    ...
    「写真番号1-1」「写真番号1-2(1-1)」などを対象とし、
    括弧内（前回番号）は無視する。
    """
    text = _normalize_text(text)
    results = set()

    # 「写真番号N-M（括弧内は前回番号）」パターン
...
def _parse_photo_page_nums(text):
    ...
    for m in RE_PHOTO_PAGE_NUM.finditer(text):
    ...
    base  = min(nums)
    upper = base + 10  # 1ページ最大6枚 + 余裕

    # テキストを整形してから base〜upper の孤立数字を回収
    work = re.sub(r'\d{4}[./]\d{2}[./]\d{2}', '', text)   # 撮影日除去
    work = re.sub(r'\d+\.\d+', '', work)                   # 小数除去
    work = re.sub(r'前回-?\d*', '', work)                    # 前回-N 除去
    # 「-N」形式（前回写真番号参照）を除去
...
def run_process(input_path, output_path, dpi, log_cb, done_cb):
    ...
    try:
        ...
        for pidx in diag_pages:
            ...
            if span is None:
                log_cb(f"  警告: 損傷図 p.{pidx+1} の径間番号をテキストから取得できません。OCRを試みます...")
                span = get_span_number_ocr(input_path, pidx, dpi)
            diag_span[pidx] = span
        ...
        for pidx in photo_pages:
            ...
            span = get_span_number_from_text(text)
            if span is None:
                log_cb(f"  警告: 損傷写真 p.{pidx+1} の径間番号をテキストから取得できません。OCRを試みます...")
                span = get_span_number_ocr(input_path, pidx, dpi)
        ...
        for didx in diag_pages:
            ...
            # OCRで取れない場合 or 取れた径間番号が0の場合、
            # ページ自体の径間番号で補完する
            page_span = diag_span.get(didx)
            if not pairs and page_span is not None:
                # テキストから直接 "写真番号N-M" 形式を試す
                text = reader.pages[didx].extract_text() or ""
                text = _normalize_text(text)
                for m in RE_DIAG_PHOTO_NUM.finditer(text):
        ...
        for didx, plist in diag_to_photo_pages.items():
            ...
            for pp in plist:
                ...
                span_label = f"{span}径間 " if (is_multi_span and span) else ""
                label = (f"→ {span_label}写真番号 {min(photo_nums)}〜{max(photo_nums)} を見る"
                         if photo_nums else f"→ {span_label}損傷写真 p.{pp+1} を見る")
        ...
        for pp, dlist in photo_to_diag_pages.items():
            ...
            for didx in dlist:
                ...
                span_label = f"{span}径間・" if (is_multi_span and span) else ""
                btn_list.append((f"← 損傷図（{span_label}{title}）を見る", didx))
